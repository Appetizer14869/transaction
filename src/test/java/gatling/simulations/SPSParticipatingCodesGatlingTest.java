...
/**
 * Performance test for the SPSParticipatingCodes entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.11.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class SPSParticipatingCodesGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all sPSParticipatingCodes")
                    .get("/services/transaction/api/sps-participating-codes")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new sPSParticipatingCodes")
                        .post("/services/transaction/api/sps-participating-codes")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"biccode\": \"SAMPLE_TEXT\"" +
                                ", \"bicname\": \"SAMPLE_TEXT\"" +
                                ", \"bicstatus\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_sPSParticipatingCodes_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created sPSParticipatingCodes")
                            .get("/services/transaction${new_sPSParticipatingCodes_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created sPSParticipatingCodes")
                        .delete("/services/transaction${new_sPSParticipatingCodes_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SPSParticipatingCodes entity").exec(scn);
