...
public class CBSTransactionsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all cBSTransactions")
                    .get("/services/transaction/api/cbs-transactions")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new cBSTransactions")
                        .post("/services/transaction/api/cbs-transactions")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                ...
                                ", \"creditorsname\": \"SAMPLE_TEXT\"" +
                                ", \"creditorsaccountid\": \"SAMPLE_TEXT\"" +
                                ", \"creditorsphone\": \"SAMPLE_TEXT\"" +
                                ...
                                ", \"cbsrequestxml\": \"SAMPLE_TEXT\"" +
                                ", \"cbsresponsexml\": \"SAMPLE_TEXT\"" +
                                ", \"amount\": 0" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_cBSTransactions_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created cBSTransactions")
                            .get("/services/transaction${new_cBSTransactions_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created cBSTransactions")
                        .delete("/services/transaction${new_cBSTransactions_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CBSTransactions entity").exec(scn);
