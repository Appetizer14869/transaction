...
/**
 * Performance test for the SPSIncomingTransactions entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.11.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class SPSIncomingTransactionsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all sPSIncomingTransactions")
                    .get("/services/transaction/api/sps-incoming-transactions")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new sPSIncomingTransactions")
                        .post("/services/transaction/api/sps-incoming-transactions")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_sPSIncomingTransactions_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created sPSIncomingTransactions")
                            .get("/services/transaction${new_sPSIncomingTransactions_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created sPSIncomingTransactions")
                        .delete("/services/transaction${new_sPSIncomingTransactions_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SPSIncomingTransactions entity").exec(scn);
